...
const detail = ref<GithubReleaseDetail | null>(null);
...
async function loadRelease() {
  ...
  try {
    await props.runAction(async () => {
      const loaded = await getReleaseDetail(props.path, props.release.tagName);
      if (currentRequest === requestId) detail.value = loaded;
    }, `加载 Release ${props.release.tagName}`);
  } catch {
...
async function saveRelease() {
  const currentTag = props.release.tagName;
...
watch(() => [props.path, props.release.tagName] as const, loadRelease, { immediate: true });
</script>

<template>
  <section class="detail-object-content" aria-label="Release 详情" :aria-busy="(!detail && !detailError) || busy">
    <div class="detail-object-summary">
      <span class="section-label">RELEASE {{ release.tagName }}</span>
      <h3>{{ release.name ?? release.tagName }}</h3>
      <p>{{ release.isLatest ? "Latest" : release.isDraft ? "Draft" : release.isPrerelease ? "Prerelease" : "Release" }}</p>
    </div>
    <template v-if="detailError">
      <p class="github-pr-detail-error" role="alert">{{ detailError }}</p>
      <button data-action="retry-release-detail" class="small-action retry-action" :disabled="busy" @click="loadRelease">
        重试加载
      </button>
    </template>
